...
class TestSessionCreate:
    ...
    def test_create_invalid_params_clamped(self, create_session):
        # 非法参数应被钳制到合法范围而不是报错
        sid = create_session(target_count=999, vuln_density=-1, defense_strength="banana")
        assert sid


# ── 对抗回合 ──────────────────────────────────────────
class TestRounds:
    def test_run_round_returns_decision(self, create_session, app):
        sid = create_session()
        c = app.test_client()
    ...
    def test_run_all_completes(self, create_session, app):
        sid = create_session()
        c = app.test_client()
    ...
    def test_unknown_session_404(self, app):
        c = app.test_client()
        resp = c.post("/api/ai/session/not_exist/round")
        assert resp.status_code == 404


# ── 态势感知（迁移自 agent_coordinator 的独有能力） ──
class TestSituationAwareness:
    def test_real_time_data_populated(self, create_session, app):
        sid = create_session()
        c = app.test_client()
    ...
    def test_threat_level_valid(self, create_session, app):
        sid = create_session()
        c = app.test_client()
        c.post(f"/api/ai/session/{sid}/run")
        resp = c.get(f"/api/ai/session/{sid}")
    ...
    def test_strategy_adjustment_recorded(self, create_session, app):
...
class TestEvaluation:
    def test_evaluate_has_confusion_matrix(self, create_session, app):
        sid = create_session()
        c = app.test_client()
        c.post(f"/api/ai/session/{sid}/run")
    ...
    def test_report_markdown(self, create_session, app):
        sid = create_session()
        c = app.test_client()
        c.post(f"/api/ai/session/{sid}/run")
    ...
    def test_report_html(self, create_session, app):
        sid = create_session()
        c = app.test_client()
        c.post(f"/api/ai/session/{sid}/run")
    ...
    def test_analyze_deep_evaluation(self, create_session, app):
