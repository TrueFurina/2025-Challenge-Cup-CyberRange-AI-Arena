...
from .models import Attack, Defense, Role, Tool, User, Vulnerability
        ...
        if not re.match(r"^[a-zA-Z][a-zA-Z0-9_]{2,15}$", username):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "用户名必须以字母开头，3-16位字母数字下划线",
                    }
                ),
                400,
            )
...
def reset_user_password(user_id):
    ...
    try:
        ...
        if not new_password:
            # P2 修复：临时密码加强（大小写+数字，长度 12），避免弱格式被暴力猜测
            import random
            import string
        ...
        db.session.commit()
        return jsonify(
            {"success": True, "message": "密码已重置", "temp_password": new_password}
        )
    except Exception as e:
        db.session.rollback()
...
def metrics():
    ...
    try:
        ...
        # 系统指标
        metrics_data.append("# HELP system_cpu_percent CPU使用率")
        metrics_data.append("# TYPE system_cpu_percent gauge")
        metrics_data.append(f"system_cpu_percent {cpu_percent}")

        metrics_data.append("# HELP system_memory_percent 内存使用率")
        metrics_data.append("# TYPE system_memory_percent gauge")
        metrics_data.append(f"system_memory_percent {memory.percent}")

        metrics_data.append("# HELP system_disk_percent 磁盘使用率")
        metrics_data.append("# TYPE system_disk_percent gauge")
        metrics_data.append(f"system_disk_percent {disk.percent}")

        # 应用指标
        metrics_data.append("# HELP flask_requests_total 请求总数")
        metrics_data.append("# TYPE flask_requests_total counter")
        for endpoint, count in request_count.items():
            metrics_data.append(
                f'flask_requests_total{{endpoint="{endpoint}"}} {count}'
            )
        ...
        try:
            ...
            metrics_data.append("# HELP database_records_total 数据库记录总数")
            metrics_data.append("# TYPE database_records_total gauge")
            metrics_data.append(
                f'database_records_total{{table="attacks"}} {attack_count}'
            )
            metrics_data.append(
                f'database_records_total{{table="defenses"}} {defense_count}'
            )
            metrics_data.append(f'database_records_total{{table="tools"}} {tool_count}')
            metrics_data.append(
                f'database_records_total{{table="vulnerabilities"}} {vuln_count}'
            )
        except Exception:
            pass
    ...
    try:
        ...
        for comp in competitions:
            start_time = datetime.fromisoformat(
                comp["startTime"].replace("Z", "+00:00")
            )
            end_time = datetime.fromisoformat(comp["endTime"].replace("Z", "+00:00"))

            if server_time < start_time:
                comp["status"] = 0  # 未开始
                comp["time_until_start"] = int(
                    (start_time - server_time).total_seconds()
                )
