...
class SystemMonitor:
    ...
    def check_service_health(self):
        ...
        # 计算整体健康状态
        healthy_services = sum(1 for status in services.values() if status['status'] == 'healthy')
        total_services = len(services)
        overall_health = 'healthy' if healthy_services == total_services else 'warning' if healthy_services > total_services // 2 else 'critical'
    ...
    def _check_web_server(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {'status': 'warning', 'message': f'Web服务器响应异常: {response.status_code}'}
        except Exception as e:
    ...
    def _check_disk_space(self):
        ...
        try:
            ...
            if usage_percent < 80:
                ...
                return {'status': 'warning', 'message': f'磁盘使用率较高: {usage_percent:.1f}%'}
            else:
                return {'status': 'critical', 'message': f'磁盘空间不足: {usage_percent:.1f}%'}
        except Exception as e:
    ...
    def _check_memory_usage(self):
        ...
        try:
            ...
            if memory.percent < 80:
                return {'status': 'healthy', 'message': f'内存使用率: {memory.percent:.1f}%'}
    ...
    def _backup_database(self):
        ...
        try:
            import shutil
            db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'app.db')
            backup_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), f'app_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.db')
